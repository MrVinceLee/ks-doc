...
1）第 一代计算机 (1946-—1957 年)——电子管时代。特点:逻辑元件采用电子管;使用机 器 语 言 进 行 编 程 ; 主存 用 延 迟 线 或 磁 鼓 存 储 信 息， 容 量 极 小 ; 体 积 庞 大 ， 成 本 高 ; 运 算 速度较低， 一般只有几千次到儿万次每秒。

2）第二代计算机 (1958-- 1964 年)——晶体管时代。特点:逻辑元件采用晶体管;运算速 度提高到几万次到几 十万次每秒; 主存使用磁心存储器;软件开始使用高级语言，如F O R T R A N ， 有 了操 作 系 统 的 维 形 

3）第三代计算机(1965-—1971 年)——中小规模集成电路时代。特点:逻辑元件采用中小

规模集成电路;半导体存储器开始取代磁心存储器;高级语言发展迅速，操作系统也进 一步 发 展 ， 开 始 有 了 分 时 操 作 系 统 。

4）第四代计算机(1972 年至今)一—超大规模集成电路时代。特点:逻辑元件采用大规模 集成电路和超大规模集成电路，产生了微处理器:诸如并行、流水线、高速缓存和虚找 存储器等概念用在 了这代计算机中。
...
电 子计 算 机 可 分 为 电 子模 拟 计 算 机 和 电 子数 字 计 算 机 。
